    ...
    methods
        ...
        function obj = runBacktest(obj, hurdleTT, pricesTT, varargin)
            ...
            for i = 1:numel(strategies)
                ...
                TW_Return_Full.(stratName) = zeros(numTimeSteps, obj.NumAssets);  % Cash and all assets
                MD_Return_Full.(stratName) = zeros(numTimeSteps, obj.NumAssets);  % Cash and all assets
                ...
                if isempty(Transaction)
                    Transaction_value = zeros(numTimeSteps, obj.NumAssets);
                else
                    Transaction_value = Transaction.Variables;
                end
                ...
                for btIdx = 1:numTimeSteps
                    ...
                    if 0 <= sodPositions(cash_idx)
                        ...
                        %assetReturnsi(cash_idx) = obj.RiskFreeRate/360;
                    ...
                    % Add Transaction Value to changes in Cash Flow
                    deltaCashFlow = Transaction_value(btIdx,:);
                    ...
                    if (pricesTT.Time(btIdx) ==  NextRebalanceDate)
                        ...
                        if 1 <= endTimeIdx - strategies(i).LookbackWindow(1)
                            ...
                            startTimeIdx = endTimeIdx - strategies(i).LookbackWindow(end)  ; % + 1;
                            dataRange = max(1,startTimeIdx):min(endTimeIdx,numTimeSteps);
                            assetLookbackData = assetReturns(dataRange, assetIdx); %%% pricesTT(dataRange, :);
                            ...
                            eodAssetWeights = eodAssetWeightsUpdated;
                            
                            % Delta Cash Flow 
                            %deltaCashAccount = (sum(eodAssetWeightsUpdated)-sum(eodAssetWeights)) * eodPortValue;
                            deltaCashFlow = deltaCashFlow + deltaWeights * eodPortValue;
                    ...
                    eodPositions = eodPortValue * eodAssetWeights + Transaction_value(btIdx,:);
                    %eodPositions(cash_idx) = eodPositions(cash_idx)- buycost(btIdx, i) - sellcost(btIdx, i);
                    ...
                    % Update Total cash flow table                    
                    deltaCashFlow_array = [deltaCashFlow_array; deltaCashFlow];

                    % Calcuate Gain or Loss over this period
                    
                    MD_PNL = zeros(1,obj.NumAssets);
                    asset_ind = find(Position_start ~= 0);
                    MD_PNL(asset_ind) = eodPositions(asset_ind) - Position_start(asset_ind) - sum(deltaCashFlow_array(:,asset_ind),1);

                    % Calculate Average Cash Flow over this period
                    weightedCashFlow = zeros(1,obj.NumAssets);
                    Ave_capital = zeros(1,obj.NumAssets);

                    for col = 1: obj.NumAssets
                        last_Ind = find(deltaCashFlow_array(:,col)==-999,1,'last');
                        if isempty(last_Ind)
                            last_Ind = 0;
                        end
                        ndays = btIdx - last_Ind;
                        h = height(deltaCashFlow_array);
                        if ndays == 1
                            weightedCashFlow(col) = 0;
                        else
                            weightedCashFlow(col) = linspace(ndays-1,1,ndays-1)*deltaCashFlow_array((last_Ind+1:h-1),col)/ndays;
                        end
                    end
                    
                    Ave_capital(asset_ind) = Position_start(asset_ind) + weightedCashFlow(asset_ind);
                    if isempty(Ave_capital)
                        MD_return = zeros(1,obj.NumAssets);
                    else 
                        MD_return = MD_PNL./Ave_capital;
                    end
                    % Record Time Weighted Return
                    MD_Return_Full.(strategies(i).Name)(btIdx, :) = MD_return;
                                                            
                    % Update Start Position if is all sold
                    allSoldAsset_ind = find(eodPositions==0) ; 
                    Position_start(allSoldAsset_ind) = 0;
                    deltaCashFlow_array(:,allSoldAsset_ind) = -999; % We will not calculate this 
                    
                    % Update Start Position if it is bought back after all sold
                    newBoughtAsset_ind = find(eodPositions(Position_start==0)~=0);
                    Position_start(newBoughtAsset_ind) = eodPositions(newBoughtAsset_ind);
                end
            end
